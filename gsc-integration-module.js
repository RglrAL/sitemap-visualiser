...
// Add GSC button to mobile menu
function addMobileGSCButton() {
    ...
    if (!mobileContainer || !mobileSection) {
        debugLog('Mobile integrations container not found');
        return;
    }
    
    // Check if button already exists
    if (document.getElementById('mobileGscBtn')) {
        debugLog('Mobile GSC button already exists');
        return;
    }
    ...
    // Create mobile GSC button
    const mobileGscButton = document.createElement('button');
    mobileGscButton.className = 'mobile-nav-item';
    mobileGscButton.id = 'mobileGscBtn';
    mobileGscButton.onclick = () => {
        toggleGSCConnection();
        closeMobileMenu();
    };
    
    mobileGscButton.innerHTML = `
    ...
    mobileContainer.appendChild(mobileGscButton);
    debugLog('Mobile GSC button added');
}
        ...
        const gscHtml = `
            <div style="background: linear-gradient(135deg, #f8f9ff 0%, #e8f1fe 100%); padding: 16px; border-radius: 8px; border: 1px solid #e3f2fd;">
                ...
                <div style="display: flex; gap: 8px; justify-content: space-between; margin-top: 12px;">
                    ...
                    <button onclick="window.showUnifiedDashboardReport && window.showUnifiedDashboardReport('${gscData.url}')" 
    ...
    function showGSCReadyMessage() {
        ...
        message.innerHTML = `
            <div style="display: flex; align-items: center; gap: 10px;">
                <span style="font-size: 20px;"><svg id="gscIcon" width="28" height="28" viewBox="0 0 24 24" style="flex-shrink: 0;">
                <path fill="#4285f4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34a853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#fbbc05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#ea4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
...
function createEnhancedDashboardHTML(url, gscData, ga4Data, gscTrends, ga4Trends, trafficSources, deviceData) {
    ...
    // The "Full Analysis" button opens the unified dashboard (showUnifiedDashboardReport).
    // Legacy showDetailedGSCAnalysis + generateEnhancedDetailedAnalysisHTML removed (unused).
