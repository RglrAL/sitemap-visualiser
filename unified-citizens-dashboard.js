...
window.DashboardGlossary = {
    ...
    // Specifically enable for unified citizens dashboard
    enableForUnifiedDashboard: function() {
        console.log('🎯 Enabling glossary for Unified Citizens Dashboard...');
        
        // Mark as unified dashboard
        if (!window.UnifiedCitizensDashboard) {
            window.UnifiedCitizensDashboard = true;
        }
        
        // Force show FAB
        this.forceShowFAB();
        
        console.log('✅ Glossary enabled for Unified Citizens Dashboard');
    },
...
(function() {
    ...
    // Check if FAB should be shown - ONLY on unified citizens dashboard
    function shouldShowFAB() {
        // Method 1: Check for unified dashboard script
        const unifiedDashboardScript = Array.from(document.scripts).some(script => 
            script.src.includes('unified-citizens-dashboard') || 
            script.textContent.includes('unified-citizens-dashboard.js') ||
            script.textContent.includes('Loading Unified Citizens Dashboard')
        );
        
        // Method 2: Check for specific dashboard elements/globals that your dashboard creates
        const hasUnifiedDashboard = 
            document.querySelector('.dashboard-container') ||
            document.querySelector('[data-dashboard="unified-citizens"]') ||
            window.UnifiedCitizensDashboard ||
            document.title.toLowerCase().includes('citizens dashboard');
        
        // Method 3: Check URL contains specific path for your dashboard
        const currentPath = window.location.pathname.toLowerCase();
        const isUnifiedDashboardPage = 
            currentPath.includes('citizens-dashboard') ||
            currentPath.includes('unified-dashboard') ||
            currentPath === '/dashboard' && hasUnifiedDashboard;
        
        const shouldShow = unifiedDashboardScript || hasUnifiedDashboard || isUnifiedDashboardPage;
        
        if (CONFIG.DEBUG) {
            debugLog(`📍 FAB Visibility Check:`, {
                unifiedScript: unifiedDashboardScript,
                hasElements: hasUnifiedDashboard,
                correctPath: isUnifiedDashboardPage,
        ...
        try {
            ...
            instance.init().then(success => {
                if (success) {
                    ...
                    console.log('  DashboardGlossary.enableForUnifiedDashboard() - Enable for your dashboard');
                    ...
                    if (fabVisible) {
                        console.log('  ✅ FAB visible - Unified Citizens Dashboard detected');
                    } else {
                        console.log('  ℹ️ FAB hidden - Not on Unified Citizens Dashboard');
                        console.log('  💡 This is expected on other pages');
                        console.log('  🔧 Run DashboardGlossary.enableForUnifiedDashboard() to enable');
                    }
                    console.log('');
